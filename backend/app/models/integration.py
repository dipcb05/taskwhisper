...
class Integration:
    ...
    @classmethod
    async def upsert(cls, db: Database, user_id: str, provider: str, config: dict[str, Any]) -> None:
    ...
    @classmethod
    async def list_for_user(cls, db: Database, user_id: str) -> list[dict[str, Any]]:
        cursor = db[cls.collection].find({"user_id": user_id})
        return [doc async for doc in cursor]

    @classmethod
    async def get(cls, db: Database, user_id: str, provider: str) -> dict[str, Any] | None:
